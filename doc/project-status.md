...
- 玩家可以无限重新游玩；每个本地自然日只有 1 个公开席位，当天的新作品可替换今晚已有作品。
- 个人最多保存最近 20 张作品；社区墙展示合并后的最新 24 张。
- 墙卡可以点击打开完整详情。
- 其他用户的头像和姓名可打开 Aigram 主页。
- 本人作品支持两次确认删除；删除当前作品后，可用新作品重新占用今晚的席位。
...
- 每日席位日期只读取 `lastBroadcastDay`，当前作品由 `lastBroadcastId` 标记，不读取平台跨日统计。
- 当天再次发表会原子式替换当前席位作品，不增加墙条目；旧存档缺少 `lastBroadcastId` 时回退到个人作品列表第一条。
...
- 每日席位专项回归通过：首次发表、当天第二次替换、墙上保持 1 张、删除变为 0 张、重新调频后再次发表。
- 结算与详情的视觉世界一致。
...
- 本次修复：每日发表从“整天硬锁”改为“单一可替换公开席位”，并补充删除后重新发表路径。
