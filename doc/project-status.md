...
更新日期：2026-07-13
当前状态：V2 作品系统已正式上线；每日生成与自动播出规则已完成
...
- 每个本地自然日只能生成 1 张作品；完成后作品自动保存并公开播出，不再提供独立发表按钮。
- 个人最多保存最近 20 张作品；社区墙展示合并后的最新 24 张。
- 墙卡可以点击打开完整详情。
- 其他用户的头像和姓名可打开 Aigram 主页。
- 本人作品支持两次确认删除；删除不会返还当天生成额度。
...
- 每日生成门禁只读取 `lastGeneratedDay`，不读取平台跨日统计；按钮、键盘 Enter 和 hook 内部 `start()` 使用同一门禁。
- 生成完成即写入 `lastGeneratedId` 和 `broadcasts` 并自动公开播出；旧存档缺少新字段时回退到 `lastBroadcastDay`、`lastBroadcastId` 或当日作品列表第一条。
...
- 英文与中文基础流程均通过：开始、5 轮选择、结算、发布、进入墙、打开详情。
- 每日生成专项回归通过：首次完成自动发表、无手动发表按钮、当天无法再次生成、删除不返还额度、刷新后门禁保持、键盘无法绕过。
- 结算与详情的视觉世界一致。
...
- `96a7e03`：短暂将每日发表改为可替换公开席位。
- 本次修正：限制对象改为每日生成额度，作品完成后自动播出；删除不返还额度。
