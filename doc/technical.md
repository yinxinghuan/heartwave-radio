...
- `src/HeartwaveRadio/HeartwaveRadio.tsx`：三态主界面、平台存档本地镜像、每日生成门禁、完成后自动播出和墙页面切换。
- `src/HeartwaveRadio/hooks/useHeartwave.ts`：题目抽取、倒计时、连击、计分、频道推导、最高分和键盘输入；`canStart` 同时约束按钮与 Enter 键入口。
...
- `useGameSave` 的 `savedData` 只用于首次加载，之后以组件内 `local` 状态作为唯一读写镜像；每次生成并自动播出时保存最近 20 张个人频率卡。
...
- 每个本地自然日只有一次生成额度：`HeartwaveSave.lastGeneratedDay` 标记消耗日期，`lastGeneratedId` 标记该次作品。游戏完成后主组件的 effect 同步把作品加入 `broadcasts`、写入生成日期并触发播出音效，不存在独立发表步骤；首页按钮和 `useHeartwave(canStart)` 共同阻止当天再次生成。删除只移除 `broadcasts` 条目，不清除生成日期。旧存档缺少新字段时回退到 `lastBroadcastDay`、`lastBroadcastId` 或当日作品列表第一条，无需批量迁移。
...
- 调整每日生成额度、自动播出、个人存档容量或墙展示上限：分别修改 `HeartwaveRadio.tsx` 的 `hasGeneratedToday` / 完成 effect、20 条上限，以及 `components/Wall.tsx` / `hooks/useWall.ts` 的 24 条上限；所有开始入口继续通过 `useHeartwave(canStart)` 统一门禁。
