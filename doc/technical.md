...
- `src/HeartwaveRadio/components/FrequencyCard.tsx`：结果页与社区墙共用的响应式频率卡，根据频道输出 warm / wild / quiet / bright 四套视觉模板类与纹理节点。
...
- 社区数据钩子遍历每个用户存档内全部 `broadcasts`，跨用户按 `createdAt` 排序；墙组件将本地个人条目先行合并，按 `broadcast.id` 去重后展示 24 条。
