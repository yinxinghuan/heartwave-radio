...
export default function HeartwaveRadio(){
  ...
  return <main className="hw"><div className="hw__orb hw__orb--one"/><div className="hw__orb hw__orb--two"/><div className="hw__grain"/>
    ...
    {game.isGameOver&&<section className="hw-result"><header className="hw-result__intro"><small>{t('result')}</small></header><Artwork broadcast={game.broadcast}/>{game.broadcast.transmission&&<blockquote className="hw-transmission">“{game.broadcast.transmission}”<small>{game.broadcast.signature}</small></blockquote>}<div className="hw-score"><span>{t('score')} <b>{game.score}</b></span><i/><span>{t('best')} <b>{game.bestScore}</b></span></div><button className={`hw-cta hw-cta--publish ${(published||hasBroadcastToday)?'is-done':''}`} disabled={published||hasBroadcastToday} onPointerDown={publish}>{published?'✓ '+t('published'):hasBroadcastToday?'◷ '+t('broadcastTomorrow'):'↗ '+t('publish')}</button><p className="hw-daily-note">{t('dailyRitual')}</p><div className="hw-result__row"><button onPointerDown={()=>setScreen('wall')}>{t('viewWall')}</button><button onPointerDown={game.start}>{t('again')}</button></div><button className="hw-home" onPointerDown={game.home}>← {t('home')}</button></section>}
