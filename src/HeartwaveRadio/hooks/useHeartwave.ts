...
export function useHeartwave(canStart=true){
 ...
 const start=useCallback(()=>{if(!canStart)return;resumeAudio();startSound();const qs=shuffle();setQuestions(qs);setRound(0);setPicks([]);picksRef.current=[];setScore(0);scoreRef.current=0;setCombo(0);setTimeLeft(DURATION);setIsPlaying(true);setIsGameOver(false);roundStart.current=performance.now();window.clearInterval(timer.current);timer.current=window.setInterval(()=>setTimeLeft(v=>{if(v<=1){window.clearInterval(timer.current);setTimeout(()=>finish(picksRef.current,scoreRef.current,true),0);return 0}return v-1}),1000)},[canStart,finish]);
